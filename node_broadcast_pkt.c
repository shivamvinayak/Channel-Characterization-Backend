...
void link_learn()
{
	...
	per_list_ptr = fopen("textfiles/per_info.txt", "a");
	rssi_list_ptr = fopen("textfiles/rssi_info.txt", "w");
	...
	printf("\nNodes that are going to perform link learning are:\t");
	for (i = 0; i < MAX_NUM_NODES + 1; i++) {
...
//	unsigned short numofpkts = 0;
	...
	fclose(InputFp);
//      ptr->type = START_LINK_LEARNING_PKT;
	ptr->numofpkts = numofpkts;
//      write(BS_sock, &buffer, sizeof(message_t));             

//      printf("LL Configuration: power-%d number of packets- %u \n",ptr->power, ptr->numofpkts);
...
// Printing IP addresses of nodes that are going to perform Link Learning..........
	printf("Printing the IP addresses of nodes that are going to perform Link learning... \n");
	for (i = 0; i < MAX_NUM_NODES + 1; i++) {
		if (sources[i] != 0)
			printf("%s\n", sources_ip[sources[i] - 1]);
	}

/* We take the indices of the nodes read from the file and use that element of the ip address file*/
// inserting code for socket comm for link learning......
	...
	for (j = 0; j < MAX_NUM_NODES; j++) {
		...
		if (sources[j] != 0) {
			printf("\nSending LL command to the node with IP address- %s \n",sources_ip[sources[j] - 1]);
...
// binding to the socket
			...
			//inet_pton(AF_INET6, SERVADDR, &server_addr.sin6_addr);  /* the server IP address, in network byte order */a
			...
			for (retrans = 0; retrans < MAX_RETRANS; retrans++) {
				...
				if (sendto(sock, ptr, sizeof(config_pkt), 0,(struct sockaddr*)&server_addr,sizeof(server_addr)) < 0) {
				...
				if (pollres == -1)
					...
					printf
						("Timeout occurred! No data till 10 seconds.\n");
					continue;
				} else {
			...
			printf("Waiting for a specified amount of time..\n");
			sleep(120);
			close(sock);
// inserted code regarding link learning ends here
			printf("moving on to the next node \n ");
		}
	}
	printf("Ending the link learning procedure \n ");
}
